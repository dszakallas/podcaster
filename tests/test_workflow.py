...
def test_generate_download_and_tag_podcast_gcp_config_passed():
    async def _test():
        custom_gcp = GCPConfig(project_id="test-proj", gcs_bucket="test-bucket", location="us-central1")
        ...
        with patch("podcaster.workflows.deep_dive_article.workflow.transcription.create_transcription_jobs", side_effect=mock_create_jobs_gen) as mock_create_jobs, \
             patch("podcaster.workflows.deep_dive_article.workflow.transcription.poll_transcription_jobs", side_effect=mock_poll_jobs_gen), \
             patch("podcaster.workflows.deep_dive_article.workflow.transcription.download_transcription_jobs", side_effect=mock_dl_jobs_gen), \
             patch("podcaster.workflows.deep_dive_article.workflow.os.path.exists", return_value=True), \
             patch("podcaster.workflows.deep_dive_article.workflow.audio_gen_core.poll_tasks", side_effect=mock_poll), \
             patch("podcaster.workflows.deep_dive_article.workflow.audio_gen_core.download_artifacts", side_effect=mock_dl):
