...
def build_distribution(
    ...
    if isinstance(dist_input, str):
        ...
        if dist_input.ref:
            ...
            if ref in visited_refs:
                raise ValueError(f"Circular reference detected in distributions: {ref}")
            if ref not in config.distributions:
                raise ValueError(f"Distribution '{ref}' not found in configuration.")
            visited_refs.add(ref)
            dist_cfg = config.distributions[ref]
        else:
            dist_cfg = None
    ...
    if rsync_cfg is not None:
        ...
        if (
            isinstance(rsync_cfg, RsyncDistributionRef)
            and rsync_cfg.ref
            and not rsync_cfg.destination
        ):
        ...
        if (
            isinstance(plex_cfg, PlexDistributionRef)
            and plex_cfg.ref
            and plex_cfg.section_id is None
        ):
